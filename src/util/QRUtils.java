...
public class QRUtils {
    ...
    public static void AddInfoToPDF(Paper paper, File pdf, File img) throws WriterException, IOException {
        ...
        final String content = paper.getAuthor() + " (" + paper.getPublish_date().replaceAll("-0?", ".") + "). " + paper.getTitle() + ".";
        ...
        stream.newLineAtOffset(WIDTH + 2 * FONTSIZE, page.getMediaBox().getHeight() - 0.75F * FONTSIZE - (float) HEIGHT / 2);
        ...
        stream.beginText();
        stream.setFont(PDType1Font.HELVETICA, FONTSIZE);
        stream.newLineAtOffset(WIDTH + 2 * FONTSIZE, page.getMediaBox().getHeight() - 2.25F * FONTSIZE - (float) HEIGHT / 2);
